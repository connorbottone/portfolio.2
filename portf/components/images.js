...
const products = [
  {
    ...
    { name: 'TETROMANIA', image: '/tetris.webm', description:  "This webpage was desgined for users to play tetris solo and multiplayer. The user has the opton to load into classic tetris that monitors there score level and plays background musci that can be toggled on and off. The user also hs the option to play multiplayer tetris. The user can play wit a freind localy in a 1v1 using the same input devicxe. One user can use the arow keys to change ther game board with the other user using the AWSD.", webpagelink: "https://localtetris.herokuapp.com/",
